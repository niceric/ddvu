...
def analyze_applications(applications, job_description):
    messages = [
        {"role": "system", "content": "You are an assistant that helps to evaluate job applications."},
        {"role": "user", "content": f"Användaren har gett dig {len(applications)} filer som är jobbansökningar och 1 fil som är själva jobbannonsen som ansökningarna är till. \
...
if st.button("Analysera") and uploaded_files and job_ad_file:
    ...
    if None not in applications and job_description:
        ...
        if result:
            ...
            qualified = [cand for cand in candidates if cand.get('qualified') == True]
            not_qualified = [cand for cand in candidates if cand.get('qualified') == False]
