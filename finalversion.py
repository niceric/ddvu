...
st.markdown(
    ...
    .main-content, .uploader, .text {{
        display: flex;
        flex-direction: column;
        align-items: center;
        color: white;
        border-radius: 12px; /* Rounded corners for main content area */
    ...
    /* Styling for the specific button */
    button.st-emotion-cache-7ym5gk.ef3psqc12 {{
        color: white; /* Setting text color to white */
        background-color: #99D90E; /* Optional: changing background color for visibility */
        border-radius: 4px; /* Optional: adjusting border radius for this specific button */
    }}
    /* Additional styles for summary container */
    summary.st-emotion-cache-p5msec.eqpbllx2 {{
        background-color: rgba(255, 255, 255, 0.5); /* Semi-transparent white background */
        color: white; /* Text color for contrast */
        border-radius: 10px; /* Rounded corners for summary */
    }}
    summary.st-emotion-cache-p5msec.eqpbllx2 span.st-emotion-cache-1dtefog.eqpbllx3 {{
        color: inherit; /* Inherits text color from parent summary */
    }}
    svg.eyeqlp51.st-emotion-cache-fblp2m.ex0cdmw0 {{
        fill: white !important;  /* Ensuring SVG is white */
    }}
   /* Styling for the specific SVG and paths within the button */
    button.st-emotion-cache-1lp7pgu.ef3psqc9 svg.eyeqlp51.st-emotion-cache-9ilocf.ex0cdmw0 path[d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6-6-6z"], 
    button.st-emotion-cache-1lp7pgu.ef3psqc9 svg.eyeqlp51.st-emotion-cache-9ilocf.ex0cdmw0 path[d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12l4.58-4.59z"] {{
        fill: white; /* Setting path color to white */
    }}
...
st.markdown(
    ...
    .uploadedFileName, .uploadedFileName * {
        color: white !important;  /* Specific targeting for uploaded file names */
    }
    .st-emotion-cache-1aehpvj, .st-emotion-cache-1aehpvj * {
        color: white !important;  /* Specific targeting for small tag */
    }
    </style>
    """,
    unsafe_allow_html=True
...
Ditt uppdrag är att noggrant granska och jämföra innehållet i dessa ansökningsfiler mot kraven specificerade i jobbannonsen för att identifiera de mest lämpliga kandidaterna. Du bör prioritera och utvärdera kandidaterna baserat på följande kriterier i angiven ordning: utbildning, erfarenheter och personliga egenskaper (framförallt de som du hittar i följande fil {(dataset_words)} som innehåller generella förmågor som efterfrågas inom vården). Inkludera hur du tänkt här i respektive ansökans förklaring/explanation. \
...
if st.button("Analysera"):
    if not uploaded_files or len(uploaded_files) < 5 or not job_ad_file:
        st.warning("Du måste ladda upp minst 5 jobbansökningar och 1 jobbannons för att kunna analysera.")
    else:
